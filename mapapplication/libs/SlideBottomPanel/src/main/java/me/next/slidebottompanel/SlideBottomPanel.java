...
public class SlideBottomPanel extends FrameLayout {
    ...
    private boolean mHidePanelTitle = true;
...
//                childView.layout(0, 0, childView.getMeasuredWidth(), childView.getMeasuredHeight());
                ...
                Log.d("measureheightonLayout", Integer.toString(mMeasureHeight));
                Log.d("mTitleHeightNoDisplay", Float.toString(mTitleHeightNoDisplay));
                Log.d("childViewHeight", Float.toString(childView.getMeasuredHeight()));
...
//                }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.d("measureheight", Integer.toString(mMeasureHeight));
    }
    ...
    private void handleActionMove(MotionEvent event) {
        if (isPanelShowing = true) {
            isPanelShowing = false;
            Log.d("handleActionMove", "true");
            if (isPanelShowing && downY < mMeasureHeight - mPanelHeight && Math.abs(event.getY() - firstDownY) > 110) {
                hidePanel();
            }
            if (!isPanelOnTouch) {
                Log.d("APositionIsTouch", "");
                return;
            }
            if (isPanelShowing && supportScrollInView((int) (firstDownY - event.getY()))) {
                Log.d("BPositionIsTouch", "");
                return;
            }
            computeVelocity();
            if (Math.abs(xVelocity) > Math.abs(yVelocity)) {
                Log.d("CPositionIsTouch", "");
                isDragging = true;
                return;
            }
            if (!isDragging && Math.abs(event.getY() - firstDownY) > mTouchSlop
                    && Math.abs(event.getX() - firstDownX) < mTouchSlop) {
                Log.d("DPositionIsTouch", "");
                isDragging = true;
                downY = event.getY();
            }
            if (isDragging) {
...
//                }
        ...
        animator.start();
        isPanelShowing = true;
//        isPanelShowing = true;     enable to touch the background view
